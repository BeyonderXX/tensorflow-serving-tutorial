...
A client that talks to tensorflow_model_server loaded with rcnn model.

The client queries the service with test inputs to get predictions, and calculates the inference error rate.

Typical usage example:

   python rcnn_client.py --num_tests=100 --server=localhost:8500
"""
...
def do_inference(hostport, concurrency, num_tests):
  ...
  """
  test_data_set = numpy.array([range(seq_length)], dtype='int32')
  label = numpy.array([[3]], dtype='int64')
  ...
  for _ in range(num_tests):
    inference(test_data_set, label, stub, result_counter)
  rate = result_counter.get_error_rate()
  print("\n")
  # print("Cost {0} seconds to predict {1} instance with {2} threads".format(time.clock()-start_time, FLAGS.num_tests, FLAGS.concurrency))

  return rate


def inference(test_data_set, label, stub, result_counter):
  request = predict_pb2.PredictRequest()
  request.model_spec.name = 'rcnn'
  request.model_spec.signature_name = 'rcnn_sig_def'
  request.inputs['input_x'].CopyFrom(
    tf.make_tensor_proto(test_data_set, shape=[1, test_data_set[0].size]))
  request.inputs['keep_prob'].CopyFrom(
    tf.make_tensor_proto(numpy.array(1.0, dtype='float32'), shape=[1]))
