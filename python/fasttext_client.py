...
A client that talks to tensorflow_model_server loaded with fasttext model.

The client queries the service with test inputs to get predictions, and calculates the inference error rate.

Typical usage example:

    python fasttext_client.py --num_tests=100 --server=localhost:8500
"""
...
def do_inference(hostport, concurrency, num_tests):
  ...
  label = numpy.array([[2]], dtype='int64')
...
def inference(test_data_set, label, stub, result_counter):
  ...
  request.model_spec.signature_name = 'fastText_sig_def'
...
def main(_):
  if FLAGS.num_tests > 100000:
    print('num_tests should not be greater than 10W')
    return
  if not FLAGS.server:
