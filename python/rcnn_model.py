...
pb_dir = "./model/rcnn/pb/"
log_file = "./model/rcnn/pb/log.txt"
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            for epoch in range(epochs):
                ...
                # test data
                feed_dict = {model.input_x: test_data,
                             model.keep_prob: 1.0}
                rst = sess.run([model.logits, model.y_pred_cls], feed_dict=feed_dict)
                ...
                if epoch < 1:
                    predict_sig_def = (saved_model.signature_def_utils.build_signature_def(
                        inputs={"input_x": saved_model.build_tensor_info(model.input_x),
                                "keep_prob": saved_model.build_tensor_info(model.keep_prob)},
                        outputs={"score": saved_model.build_tensor_info(model.logits),
                                 "out_y": saved_model.build_tensor_info(model.y_pred_cls)},
                        method_name=saved_model.signature_constants.PREDICT_METHOD_NAME
                    ))
                    builder.add_meta_graph_and_variables(sess, ["serve"],
                                                         signature_def_map={"rcnn_sig_def": predict_sig_def},
...
def test():
    ...
    graph_path = "./model_old/rcnn/model_old-11000.meta"
    model_path = "./model_old/rcnn"
    graph = tf.Graph()
