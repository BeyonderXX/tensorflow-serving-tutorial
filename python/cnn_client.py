...
A client that talks to tensorflow_model_server loaded with textCnn model.

The client queries the service with test inputs to get predictions, and calculates the inference error rate.

Typical usage example:

    python cnn_client.py --num_tests=100 --server=localhost:8500
"""
...
def inference(test_data_set, label, stub, result_counter):
  ...
  request.model_spec.signature_name = 'textCnn_sig_def'
  request.inputs['input_x'].CopyFrom(
    tf.make_tensor_proto(test_data_set, shape=[1, test_data_set[0].size]))
  request.inputs['drop_prob'].CopyFrom(
    tf.make_tensor_proto(numpy.array(0.0, dtype='float32'), shape=[1]))
